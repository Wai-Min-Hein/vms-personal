...
export function VideoPlayer({ camera }: { camera: CameraView }) {
  ...
  useEffect(() => {
    ...
    if (!video || !camera.runtime?.ready) return;
    const element = video;
    ...
    async function connectWebRtc() {
      ...
      peer.ontrack = (event) => { element.srcObject = event.streams[0]; };
      ...
      await new Promise<void>((resolve, reject) => {
        const timeout = window.setTimeout(() => reject(new Error("WebRTC connection timed out")), 8_000);
        const checkState = () => {
    ...
    return () => { cancelled = true; peerRef.current?.close(); hls?.destroy(); element.srcObject = null; };
  ...
  return (
    <div className="group relative aspect-video overflow-hidden rounded-lg border bg-black">
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/90 to-transparent p-2 opacity-0 transition-opacity group-hover:opacity-100">
        <div className="text-xs text-zinc-300">{camera.runtime?.tracks.join(" · ") || camera.streamType} · {camera.runtime?.readers ?? 0} viewers</div>
        <div className="flex">
